...
export default function IframeWindow({ element, editable, editing, onEditProps }) {
  ...
  return (
    <div className="w-full h-full rounded-2xl bg-surface/95 backdrop-blur-xl border border-edge shadow-[0_12px_40px_rgba(0,0,0,0.12)] flex flex-col overflow-hidden transition-all group">
      {/* Title bar (macOS style) */}
      <div className="relative flex items-center px-3 py-2 bg-muted border-b border-edge-subtle">
        {/* Title */}
        <div className="pointer-events-none absolute inset-0 flex items-center justify-center gap-1.5 opacity-70 group-hover:justify-start group-hover:opacity-100 transition-all duration-200 px-3 min-w-0">
          ...
          <span className="text-xs sm:text-sm font-bold tracking-wider uppercase text-content-muted truncate max-w-[calc(100%-7rem)]">
      ...
      {/* Body */}
      {showInput ? (
        <div className="flex-1 flex flex-col items-center justify-center gap-3 p-6 text-center bg-muted" onPointerDown={(e) => e.stopPropagation()}>
          <div className="w-12 h-12 rounded-2xl bg-surface shadow-sm border border-edge flex items-center justify-center text-blue-500 mb-1">
          ...
          {editable ? (
            <>
              <input
                ...
                className="w-full max-w-[20rem] text-sm bg-surface border border-edge shadow-inner rounded-xl px-4 py-2.5 outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 text-content placeholder:text-content-subtle transition-all"
              />
              <p className="text-[10px] font-bold text-content-subtle uppercase tracking-widest mt-1">Press Enter to embed</p>
            </>
          ) : (
            <p className="text-sm font-medium text-content-subtle">No embed set</p>
          )}
        </div>
      ) : (
