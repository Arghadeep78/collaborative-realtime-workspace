...
import { setPhoto } from '../../hooks/usePhotoResolver.js';
...
const Profile = () => {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            // Fall back to the previously-saved URL — never editedData.profilePic,
            // which may hold a base64 preview from handleProfilePicChange.
            let pictureUrl = userData.profilePicture ?? userData.profilePic ?? '';
            ...
            // Invalidate the shared photo cache so every Avatar in the app
            // (header, board cursors, comments) picks up the new picture without
            // a full reload. Email comes from the stored user (the PUT response omits it).
            const myEmail = normalizedUser.email || JSON.parse(localStorage.getItem('userData') || '{}').email;
            if (myEmail) setPhoto(myEmail, normalizedUser.profilePicture);
            setIsEditing(false);
