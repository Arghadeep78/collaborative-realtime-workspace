...
export const createProject = async (req, res) => {
  ...
  return sendResponse(res, 201, 'Project created', { id: project.id, title: project.title, createdAt: project.createdAt, thumbnail: project.thumbnail });
};
...
export const deleteProject = async (req, res) => {
  const project = await Whiteboard.findOne({ id: req.params.id });
  if (!project) throw new APIError(404, 'Project not found');
  if (project.owner !== req.email) throw new APIError(403, 'Only the owner can delete this project');
...
export const shareProject = async (req, res) => {
  ...
  const project = await Whiteboard.findOne({ id: req.params.id });
  if (!project) throw new APIError(404, 'Project not found');
  if (project.owner !== req.email) throw new APIError(403, 'Only the owner can share this project');
  ...
  return sendResponse(res, 200, 'Project shared', { collaborators: project.collaborators });
};
...
export const unshareProject = async (req, res) => {
  ...
  const project = await Whiteboard.findOne({ id: req.params.id });
  if (!project) throw new APIError(404, 'Project not found');
  if (project.owner !== req.email) throw new APIError(403, 'Only the owner can manage access');
  ...
  await project.save();
  await invalidateProjectMeta(project.id);
  return sendResponse(res, 200, 'Access revoked', { collaborators: project.collaborators });
};
...
export const createShareToken = async (req, res) => {
  ...
  if (project.owner !== req.email) {
    throw new APIError(403, 'Only the owner can create share links');
  }
  ...
  return sendResponse(res, 200, 'Share link created', { token, role });
};
...
export const updateProjectTitle = async (req, res) => {
  ...
  const project = await Whiteboard.findOne({ id: req.params.id });
  if (!project) throw new APIError(404, 'Project not found');
  if (project.owner !== req.email) throw new APIError(403, 'Only the owner can rename this project');

  project.title = title;
  await project.save();
  return sendResponse(res, 200, 'Title updated', { title: project.title });
};
...
export const toggleFavorite = async (req, res) => {
  ...
  await project.save();
  return sendResponse(res, 200, 'Favorite updated', { isFavorited: idx === -1 });
};
...
export const leaveProject = async (req, res) => {
  ...
  const project = await Whiteboard.findOne({ id: req.params.id });
  if (!project) throw new APIError(404, 'Project not found');
  if (project.owner === email) throw new APIError(400, 'Owners cannot leave a project; delete it instead.');
  ...
  if (!isCollaborator && !isMemberOfWorkspace) throw new APIError(400, 'You do not have access to this project.');
  ...
  await project.save();
  await invalidateProjectMeta(project.id);
  return sendResponse(res, 200, 'Left project', { ok: true });
};
...
export const updateProjectThumbnail = async (req, res) => {
  ...
  const project = await Whiteboard.findOne({ id: req.params.id });
  if (!project) throw new APIError(404, 'Project not found');
  if (project.owner !== req.email) throw new APIError(403, 'Only the owner can change the cover');

  project.thumbnail = thumbnail ?? null;
  await project.save();
  return sendResponse(res, 200, 'Cover updated', { thumbnail: project.thumbnail });
};
